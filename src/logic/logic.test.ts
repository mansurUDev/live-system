...
describe('горизонт сводки', () => {
  ...
  it('каждая строка знает id своей записи — иначе подсветить на вкладке нечего', () => {
    const items = agenda(withExpense(5), NOW)
    const row = items.find((i) => i.id === 'exp-o1')!
    expect(row.focus).toBe('o1')
    expect(row.tab).toBe('fin')
  })
